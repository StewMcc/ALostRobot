...
public class RobotController : MonoBehaviour {
		...
		if (!isTeleporting_) {
			if (isMoving_) {
				if (agent_.velocity.magnitude < stopThreshold_) {
					isMoving_ = false;
					SoundManager.StopEvent(AKID.EVENTS.PLAYER_MOVE_START, 1, gameObject);
					SoundManager.PlayEvent(AKID.EVENTS.PLAYER_MOVE_END, gameObject);
				}
			}
			...
			if (OnTap()) {
				...
				if (Physics.Raycast(ray, out hit) && !IsOverUi()) {
					...
					if (!isMoving_) {
						SoundManager.PlayEvent(AKID.EVENTS.PLAYER_MOVE_START, gameObject);
						isMoving_ = true;
					}
				}
			}
	...
	public void AddPickup(Pickup newPickup) {
		...
		SoundManager.PlayEvent(AKID.EVENTS.ITEM_PICKUP, gameObject);
		ShowInventory();
	}
	...
	private void Recall() {
		...
		SoundManager.PlayEvent(AKID.EVENTS.TELEPORT, gameObject);
